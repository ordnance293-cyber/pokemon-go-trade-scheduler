...
test('opening and saving an arranged trade resets and persists the single selection', () => {
    ...
    assert.match(script, /transaction\.update\(inventoryRef, \{ status: 'trading', partner, tradeDate: autoDate, luckyTrinket: 'seller', luckyTrinketCycleId: cycle\.id,[\s\S]*?weeklyChallengeTaskId: taskRef\.id/);
});
